...
static void CG_DrawClientScore( int y, score_t *score, float *color, float fade, qboolean largeFormat ) {
	...
	if ( ci->powerups & ( 1 << PW_NEUTRALFLAG ) ) {
		...
		if ( cgs.gametype == GT_DUEL && ci->wins) {
...
static float CG_DrawRoundTimer( float y ) {
	...
	CG_DrawBigString( 25, y + 2, s, 1.0F);
	...
	if ( cgs.gametype >= GT_TEAM ) {
		...
		if ( cg.teamScores[0] >= cg.teamScores[1] ) {
			...
			if ( cgs.gametype >= GT_RTP ) {
				...
				y += (n2b * lineHeight) + BIGCHAR_HEIGHT;
				maxClients -= n2b;
			}
			/*if ( cgs.gametype >= GT_RTP ) {
				CG_DrawTeamBackground(0, SB_TOP+lineHeight/2, 640, (n1+n1b+n2+n2b)*(lineHeight + bottomBorderSize+10),0.33, TEAM_RED);
			} else {
