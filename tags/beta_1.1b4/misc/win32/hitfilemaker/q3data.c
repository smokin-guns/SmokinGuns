...
int main (int argc, char **argv)
{
	...
	// build a md own engine ;)
	if(!strcmp(argv[1], "-mdbuild")){
		strcpy(path, argv[2]);
		ConvertMD3ToMD(path);
		return 0;
	}
	...
	if(argc < 2){
		Error ("usage: md2hit [modelpath] [tag]");
	}
	...
	if(argc >= 3){
		...
		if(!stricmp(tag, "tag_")){
			...
			printf("\nIncluding %s\n", tag);
			strcpy(path, argv[1]);
			ConvertMD3ToHit(path, tag, qtrue);
			return 0;
		}
	}

	strcpy(path, argv[1]);
	ConvertMD3ToHit(path, tag, qfalse);
