...
/*
heres the naming convention for the damage skins
...
upper:
<skinname_upper_<combination of locationnums>

lower:
<skinname_lower_<combination of locationnums>
...
void WriteSkinFiles(char* path, char* model, char* skin){
	FILE			*shader;
	char			filename[1024];
	int				i, j, k, l, m;
	int				count = 0;

	sprintf (filename, "%s/westernq3/scripts/wq3_players_%s_%s.shader", path, model, skin);
	_printf("writing %s...\n", filename);

	shader = fopen (filename, "w");

	fprintf( shader, "// damage-skin-shaders of %s\n// created by the damage-skin-script-creator of wq3\n\n", name);

	//upper parts
	for(i=1;i<=3;i++){

		//1 damage skin
		fprintf(shader, "models/wq3_players/%s/damage/%s_upper_%i\n",
			model, skin, i);
		fprintf(shader, "{\n\t{\n");

		fprintf(shader, "\t\tmap models/wq3_players/%s/damage/%s_upper_%i\n", model, skin, i);
		fprintf(shader, "\t\tblendFunc GL_SRC_ALPHA GL_ONE_MINUS_SRC_ALPHA\n");
		fprintf(shader, "\t\trgbGen lightingDiffuse\n");

		fprintf(shader, "\t}\n");
		fprintf(shader, "\t{\n");

		fprintf(shader, "\t\tmap models/wq3_players/%s/%s_upper_%s\n", model, model, skin);
		fprintf(shader, "\t\trgbGen lightingDiffuse\n");
		...
		for(j=1;j<=3;j++){
			...
			//2 damage skins
			fprintf(shader, "models/wq3_players/%s/damage/%s_upper_%i%i\n",
			model, skin, i, j);
			fprintf(shader, "{\n\t{\n");
			...
			for(k=1;k<=3;k++){
				...
				//3 damage skins
				fprintf(shader, "models/wq3_players/%s/damage/%s_upper_%i%i%i\n",
				model, skin, i, j, k);
				fprintf(shader, "{\n\t{\n");
				...
				/*for(l=1;l<=4;l++){
					...
					//4 damage skins
					fprintf(shader, "models/wq3_players/%s/damage/%s_upper_%i%i%i%i\n",
					model, skin, i, j, k, l);
					fprintf(shader, "{\n\t{\n");
					...
					for(m=1;m<=5;m++){
						...
						//5 damage skins
						fprintf(shader, "models/wq3_players/%s/damage/%s_upper_%i%i%i%i%i\n",
						model, skin, i, j, k, l, m);
						fprintf(shader, "{\n\t{\n");
...
#ifdef _SKIN
	if(argc != 4){
		Error ("usage: skin-script q3-path modelname skinname");
		return 0;
	}

	WriteSkinFiles(argv[1], argv[2], argv[3]);
	return 0;
#endif

	_printf ("Q3Map v1.0r (c) 1999 Id Software Inc.\nModified version by Spoon for Western Quake3 (c) 2001 Iron Claw Interactive\n");
