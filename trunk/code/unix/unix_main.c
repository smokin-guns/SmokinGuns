...
void Sys_PrintBinVersion( const char* name ) {
  ...
  fprintf( stdout, "\n\n%s\n", sep );
#ifdef DEDICATED
  fprintf( stdout, "%s SmokinGuns Dedicated Server [%s %s]\n", os, date, time );  
#else
...
int main ( int argc, char* argv[] )
{
  ...
  *cmdline = 0;
  ...
  // Either way, they will be initialized in Com_Init() as startup variables
    if (!strcmp(argv[i], "+set") && i+2 < argc) {
      ...
      if (!strcmp(cv_name, "sv_chroot"))
        ...
        quser = cv_value;
      else if (!strcmp(cv_name, "sv_daemon")
        && atoi(cv_value) && !errno)
        qdaemon = qtrue;
    }
#endif
  ...
  Sys_SetDefaultInstallPath(argv[0]);
  ...
  // Get pid file path
  cv_pid = Cvar_Get ("sv_pidfile", va("/var/run/%s.pid", basename(argv[0])) , CVAR_INIT);
