...
#ifdef __linux__
// compare to the comment in game/q_shared.h: only Linux/glibc
...
// In the BSD's we can and have to open it WRONLY to make multichannel possible (according to hika)
// In Linux, however, the later mmap requires the file to be opened RDWR (for BSD's write access is sufficient)
#ifdef __linux__
