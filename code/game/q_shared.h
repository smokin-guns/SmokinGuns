...
#define GAMENAME		"smokinguns"

// Home sub directories to data and config path for MAC OSX and UNIX users.
// (see ./code/unix/unix_shared.c)
#define MACOSX_HOME_SUBDIR	"/Library/Application Support/SmokinGuns"
#define UNIX_HOME_SUBDIR	"/.smokinguns"
...
typedef struct {
	...
	int			surfaceFlags;	// surface hit //changed to shaderNum
...
typedef struct playerState_s {
	...
	int			legsTimer;		// don't change to low priority animations until this runs out
	int			legsAnim;		// mask off ANIM_TOGGLEBIT

	int			torsoTimer;		// don't change to low priority animations until this runs out
...
#define BUTTON_CHOOSE_MODE	16384		// +button14, to notify the server that the client
										// is in weapon choose mode, meaning that the
										// next +attack press will be to select weapon,
										// not to fire the gun

//only used in missionpack
