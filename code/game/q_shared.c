...
// set the byte swapping variables in a portable manner
	if ( *(short *)swaptest == 1)
	{
		...
		Com_Printf("Swap_Init: byte order functions set on little endian QVM\n");
	}
	else
	{
		...
		Com_Printf("Swap_Init: byte order functions set on big endian QVM\n");
	}
...
void Info_SetValueForKey( char *s, const char *key, const char *value ) {
	...
	if(!strlen(key)){
		Com_Printf("Info_SetValueForKey: key is empty");
		return;
	}
	...
	if (strchr (key, '\"') || strchr (value, '\"'))
	{
		//Com_Printf ("Can't set keys or values with a \"\n");
		return;
	}
