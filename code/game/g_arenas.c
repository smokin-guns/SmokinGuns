...
static gentity_t *SpawnModelOnVictoryPad( gentity_t *pad, vec3_t offset, gentity_t *ent, int place ) {
	...
	switch(body->s.weapon){
	case WP_KNIFE:
	case WP_DYNAMITE:
	case WP_MOLOTOV:
		body->s.torsoAnim = TORSO_KNIFE_STAND;
		break;
	case WP_PEACEMAKER:
	case WP_REM58:
	case WP_SCHOFIELD:
		body->s.torsoAnim = TORSO_PISTOL_STAND;
		break;
	case WP_GATLING:
		body->s.torsoAnim = TORSO_GATLING_STAND;
		break;
	case WP_NONE:
		body->s.torsoAnim = TORSO_HOLSTERED;
		break;
	default:
...
static void CelebrateStop( gentity_t *player ) {
	...
	case WP_DYNAMITE:
	case WP_MOLOTOV:
		anim = TORSO_KNIFE_STAND;
		break;
	case WP_PEACEMAKER:
	case WP_REM58:
	case WP_SCHOFIELD:
		anim = TORSO_PISTOL_STAND;
		break;
	case WP_GATLING:
		anim = TORSO_GATLING_STAND;
		break;
	case WP_NONE:
		anim = TORSO_HOLSTERED;
		break;
	default:
