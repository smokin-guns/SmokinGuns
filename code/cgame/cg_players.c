...
/*int			i;
	...
	f = fopen(filename, "rb");
	if (!f)
		trap_Print (va("LoadTexInfo: couldn't read %s\n", filename));

	if (fscanf (f,"%s\n%i\n",magic, &numsurfs) != 2)
		trap_Print (va("LoadTexInfo: failed to read header"));
	if (strcmp(magic,"TEXFILE"))
		trap_Print (va("LoadTexInfo: not a tex file"));

	trap_Print (va("%i visible tris-surfaces\n", numsurfs));
...
static qboolean	CG_ParseAnimationFile( const char *filename, clientInfo_t *ci ) {
	...
	for ( i = 0 ; i < MAX_ANIMATIONS ; i++ ) {
		...
		// TODOANIM
		/*if ( i == LEGS_CROUCH_WALK ) {
			skip = animations[LEGS_CROUCH_WALK].firstFrame - animations[TORSO_GESTURE].firstFrame;
		}
...
static qboolean	CG_RegisterClientSkin( clientInfo_t *ci, const char *modelName, const char *skinName/*, const char *headModelName, const char *headSkinName*/ ) {
	...
	/*if (!ci->holsterSkin) {
		Com_Printf( "Holster skin load failure: %s\n", filename );
	}*/
...
static qboolean CG_RegisterClientModelname( clientInfo_t *ci, const char *modelName, const char *skinName ) {
	...
	/*if( headName[0] == '*' ) {
		Com_sprintf( filename, sizeof( filename ), "models/wq3_players/heads/%s/%s.md3", &headModelName[1], &headModelName[1] );
	}
	else {*/
		Com_sprintf( filename, sizeof( filename ), "models/wq3_players/%s/head.md3", modelName);//headName );
	//}
...
/*#ifdef HIT_DATA
	// load hit files
	if ( !CG_LoadHitFiles( "wq_male2", ci ) ) {
		Com_Printf( "Failed to load hit file for %s\n", "wq_male2" );
		return qfalse;
	}
#endif*/
	...
	/*if( headName[0] == '*' ) {
		Com_sprintf( filename, sizeof( filename ), "models/wq3_players/heads/%s/icon_%s.tga", &headName[1], headSkinName );
	} else */
		Com_sprintf( filename, sizeof( filename ), "models/wq3_players/%s/icon_%s.tga", modelName/*headName*/, skinName );
	//}
	...
	/*if ( !ci->modelIcon && headName[0] != '*') {
		Com_sprintf( filename, sizeof( filename ), "models/wq3_players/heads/%s/icon_%s.tga", headName, headSkinName );
		ci->modelIcon = trap_R_RegisterShaderNoMip( filename );
	}*/
...
static void CG_LoadClientInfo( clientInfo_t *ci ) {
	...
	if ( !CG_RegisterClientModelname( ci, ci->modelName, ci->skinName ) ) {
		...
		if( cgs.gametype >= GT_TEAM) {
			...
			if(ci->team == TEAM_RED || ci->team == TEAM_RED_SPECTATOR){
				strcpy( model, Info_ValueForKey( info, "g_teammodelred"));
			} else if(ci->team == TEAM_BLUE || ci->team == TEAM_BLUE_SPECTATOR){
				strcpy( model, Info_ValueForKey( info, "g_teammodelred"));
			}
	...
	for ( i = 0 ; i < MAX_CUSTOM_SOUNDS ; i++ ) {
		...
		if ( !ci->sounds[i] ) {
			ci->sounds[i] = trap_S_RegisterSound( va("sound/player/%s/%s", /*fallback*/gender, s + 1), qfalse );
		}
	}
...
void CG_NewClientInfo( int clientNum ) {
	...
	if ( !CG_ScanForExistingClientInfo( &newInfo ) ) {
		...
		if ( /*cgs.gametype < GT_RTP &&(*/forceDefer || ( cg_deferPlayers.integer && !cg_buildScript.integer && !cg.loading /*)*/) ) {
...
static void CG_SetLerpFrameAnimation( clientInfo_t *ci, lerpFrame_t *lf, int newAnimation, int weapon) {
	...
	if ( newAnimation < 0 || newAnimation >= MAX_TOTALANIMATIONS ) {
		CG_Error( "Client: Bad animation number: %i", newAnimation );
	}
...
static void CG_PlayerAnimation( centity_t *cent, int *legsOld, int *legs, float *legsBackLerp,
	...
	/*if(cent->pe.legs.animation && (cent->currentState.powerups & (1<<MV_SLOW))){
		frameLerp = cent->pe.legs.animation->frameLerp;
		cent->pe.legs.animation->frameLerp *= 2;
	}*/
	...
	/*if(cent->pe.legs.animation && (cent->currentState.powerups & (1<<MV_SLOW))){
		cent->pe.legs.animation->frameLerp = frameLerp;
	}*/
...
static void CG_HasteTrail( centity_t *cent ) {
	...
	if ( anim != LEGS_RUN && anim != LEGS_RUN_BACK ) {
		return;
	}
...
static void CG_DustTrail( centity_t *cent ) {
	...
	anim = cent->pe.legs.animationNumber & ~ANIM_TOGGLEBIT;
	if ( /*anim != LEGS_LANDB &&*/ anim != BOTH_LAND ) {
		return;
	}
...
static void CG_PlayerFlag( centity_t *cent, qhandle_t hSkin, refEntity_t *torso ) {
	...
	/*if( legsAnim == LEGS_IDLE || legsAnim == LEGS_CROUCHED_IDLE ) {
		flagAnim = FLAG_STAND;
	...
	//uncheck weapon anim
	cent->pe.flag.weaponAnim = -1;

	CG_RunLerpFrame( ci, &cent->pe.flag, flagAnim, 1, 0);
...
//	qboolean	hide = qfalse;
	...
	//if player isn't visible, don't show it
	/*if(cent->currentState.clientNum != cg.snap->ps.clientNum && !(cent->currentState.eFlags & EF_DEAD)){

		hide = CG_CheckPlayerVisible(cg.refdef.vieworg, cent->lerpOrigin);
...
#define	FIRE_SHRINK	  200
	...
	if(cent->currentState.powerups & (1 << PW_GOLD)){
		vec3_t angles;
		gitem_t *item= BG_FindItemForClassname("item_money");
		...
		money.hModel = cgs.media.e_moneybag; //cg_items[ITEM_INDEX(item)].models[0];
