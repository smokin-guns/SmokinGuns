...
static char homeq3aPath[MAX_OSPATH];
...
qboolean TestSysInstallPath(const char *path) {
	char *testpath;
	FILE *f;
...
#define xstring(x)   #x
// Packagers: change PREFIX to your favourite location
// Either here, or with the use of cons / Conscript
#ifndef PREFIX
...
// over to installPath. It would be cleaner, though, to not have this full
// function in unix_shared .... :-/
	...
	if ((wp = getenv("WQ3_BASEPATH")) != NULL) {
		Q_strncpyz(installPath, wp, sizeof(installPath));
		return;
	}
	// Keep WQ3_BASEPATH for backward compatibility,
	// but users can now rely on SG_BASEPATH
	...
	// woul work but make it non-portable ... OpenBSD ??
	if (readlink(path, real_path, MAX_OSPATH) > 0) {
...
#ifdef MACOS_X
		Q_strcat(homePath, sizeof(homePath), MACOSX_HOME_SUBDIR);
#else
		Q_strcat(homePath, sizeof(homePath), UNIX_HOME_SUBDIR);
#endif
		if (mkdir(homePath, 0777)) {
			if (errno != EEXIST) 
#ifdef DEDICATED
				Com_Printf("WARNING: Unable to create directory \"%s\", error is %s(%d)\n", homePath, strerror(errno), errno);
#else
...
char *Sys_DefaultQ3AHomePath( const char *gamedir)
{
	char *p;
	...
	if ((p = getenv("HOME")) != NULL) {
		Q_strncpyz(homeq3aPath, p, sizeof(homeq3aPath));
#ifdef MACOS_X
		Q_strcat(homeq3aPath, sizeof(homeq3aPath), "/Library/Application Support/Quake3");
#else
		Q_strcat(homeq3aPath, sizeof(homeq3aPath), "/.q3a");
#endif
		return homeq3aPath;
	}
	return ""; // assume current dir
